...
function scheduleExtractor() {
    return {
        ...
        async loadDashboard() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    window.toast.error('대시보드를 불러올 수 없습니다.');
                }
            } catch (err) {
                window.toast.error('대시보드 로드 중 오류가 발생했습니다.');
            } finally {
        ...
        validateAndSetFile(file) {
            const allowedTypes = ['.pdf', '.docx', '.doc', '.hwp', '.hwpx', '.jpg', '.jpeg', '.png', '.tiff', '.tif', '.bmp', '.webp'];
            const extension = '.' + file.name.split('.').pop().toLowerCase();

            if (!allowedTypes.includes(extension)) {
                this.error = '지원하지 않는 파일 형식입니다. PDF, DOCX, HWP, JPG, PNG 파일만 업로드 가능합니다.';
                return;
            }
        ...
        async deleteContract(contractId) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    window.toast.success('계약이 삭제되었습니다.');
                } else {
                    const data = await response.json().catch(() => ({}));
                    window.toast.error(data.detail || '계약 삭제에 실패했습니다.');
                }
            } catch (err) {
                window.toast.error('계약 삭제 중 오류가 발생했습니다.');
            }
        },
        ...
        async loadComments(contractId, taskId = null) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    window.toast.error('댓글을 불러올 수 없습니다.');
                }
            } catch (err) { window.toast.error('댓글 로드 실패'); }
        ...
        async loadActivityLogs(contractId = null) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    window.toast.error('활동 로그를 불러올 수 없습니다.');
                }
            } catch (err) { window.toast.error('활동 로그 로드 실패'); }
        ...
        getTimelineBarStyle(task) {
            ...
            const dates = tasks.map(t => new Date(t.due_date).getTime()).filter(d => !isNaN(d));
